...
def generate_bc(url, separator):
    not_acrony = ["the","of","in","from","by","with","and", "or", "for", "to", "at", "a"]
    split_url = url.split("/")
    path = split_url[1]
    
    for i in range(1,len(split_url)):
        if len(split_url[i]) >= 30:
            split_url[i] = "".join([word[0] for word in split_url[i].split("-") if word not in not_acrony])
    
    print(split_url)
    
    
    print(f'<a href="/">HOME</a>{separator}<a href="/{path}/">{split_url[1].upper()}</a>')
    return f'<a href="/">HOME</a>{separator}<a href="/{path}/">{split_url[1].upper()}</a>'